            ...
            <?php foreach($data['pcs'] as $character): ?>
                <?php if ($character === $data['pcs'][0]) : ?>
                    <div class="pcDiv" style="width: 80vw; margin: 0.5em 0 0.5em 0; padding: 0.5em 0.25em 0.25em 0.25em;">
                        <input id="pc<?=$character->pcID?>" type="radio" name="pcID" value="<?=$character->pcID?>" checked><label for="pc<?=$character->pcID?>"><span style="font-size: 14pt; font-weight: bold;"><?=$character->pcName?></span>&emsp;<p align="right" style="display: inline;">Lvl <?=$character->pcLevel?>&nbsp;<?=$character->pcRace?>&nbsp;<?=$character->pcClass?>&nbsp;<span style="color: #6A0F0F; font-weight: bold;">&#9672;</span> Max HP: <?=$character->pcMaxHP?> AC: <?=$character->pcAC?> <span style="color: #6A0F0F; font-weight: bold;">&#9672;</span> STR: <?=$character->pcSTR?> | DEX: <?=$character->pcDEX?> | CON: <?=$character->pcCON?> | INT: <?=$character->pcINT?> | WIS: <?=$character->pcWIS?> | CHA: <?=$character->pcCHA?></p></label>
                    </div>
                <?php else: ?>
                    <div class="pcDiv" style="width: 80vw; margin: 0.5em 0 0.5em 0; padding: 0.5em 0.25em 0.25em 0.25em;">
                        <input id="pc<?=$character->pcID?>" type="radio" name="pcID" value="<?=$character->pcID?>"><label for="pc<?=$character->pcID?>"><span style="font-size: 14pt; font-weight: bold;"><?=$character->pcName?></span>&emsp;<p align="right" style="display: inline;">Lvl <?=$character->pcLevel?>&nbsp;<?=$character->pcRace?>&nbsp;<?=$character->pcClass?>&nbsp;<span style="color: #6A0F0F; font-weight: bold;">&#9672;</span> HP: <?=$character->pcMaxHP?> AC: <?=$character->pcAC?> <span style="color: #6A0F0F; font-weight: bold;">&#9672;</span> STR: <?=$character->pcSTR?> | DEX: <?=$character->pcDEX?> | CON: <?=$character->pcCON?> | INT: <?=$character->pcINT?> | WIS: <?=$character->pcWIS?> | CHA: <?=$character->pcCHA?></p></label>
                    </div>
                <?php endif; ?>
            <?php endforeach; ?>
            <div align="center">
